 ...
 * @version Feb 25, 2016
 */
public class CS232LinkedBinaryTree<K, V> implements CS232BinaryTree<K, V>, Comparable {
	...
	public static class BTNode<K, V> {
		public K key;
		public V value;
		public ArrayList <Integer> dirVal;
		...
		public BTNode(K key, V value) {
			...
			right = null;
			parent = null;
			this.dirVal = new ArrayList<Integer>();
		}
	 ...
	 * 
	 * By Making the CS232LinkedBinaryTreeClass comparable, we can simply work
	 * with java's priority queue. 
	 * In this, we compare the roots for the given trees, compared first by the frequencies and then by the character
	 * (lower value has higher priority)
	 */
	@Override
	public int compareTo(Object o) {
		// cast o so we can access it's fields and methods
		CS232LinkedBinaryTree<K,V> other = (CS232LinkedBinaryTree<K,V>) o;
		
		/*
		 * Compare the roots by key (frequency)
		 */
		if ((Integer)this.root.key>(Integer)other.root.key){
			return 1;
		}else if ((Integer)this.root.key<(Integer)other.root.key){
			return -1;
		}
		else{
			/*
			 *Then compare by value (character) for a tie breaker if needed
			 */
			if ((Character)this.root.value>(Character)other.root.value){
				return 1;
			}
			else if((Character)this.root.value<(Character)other.root.value){
				return -1;
			}
			else{
				return 0;
